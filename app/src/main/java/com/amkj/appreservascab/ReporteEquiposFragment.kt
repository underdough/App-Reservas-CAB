...
class ReporteEquiposFragment : Fragment(R.layout.fragment_lista_reporte) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        adapter = ReportAdapter(tipoFijo = "Equipo")
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            try {
                ...
                if (res.isSuccessful) {
                    val body = res.body()
                    if (body != null) {
                        adapter.submit(body.equipos_top)
                        binding.tvVacio.visibility =
                            if (body.equipos_top.isEmpty()) View.VISIBLE else View.GONE
                    } else {
